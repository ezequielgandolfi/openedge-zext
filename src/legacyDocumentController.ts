...
export class LegacyABLDocument {
    ...
    public pushDocumentSignal(document: LegacyABLDocument) {
        if (this.processed) {
    ...
    public refreshDocument(): Promise<LegacyABLDocument> {
        this._processed = false;
        ...
        let result = new Promise<LegacyABLDocument>(function(resolve,reject) {
...
export class LegacyABLDocumentController {

    private _documents: LegacyABLDocument[] = [];
    ...
    public insertDocument(document: vscode.TextDocument) {
        if (document.languageId === ABL_MODE.language) {
            if (!this._documents[document.uri.fsPath]) {
                let ablDoc = new LegacyABLDocument(document);
    ...
    public removeDocument(document: vscode.TextDocument) {
        let d: LegacyABLDocument = this._documents[document.uri.fsPath];
        if (d) {
    ...
    public updateDocument(document: vscode.TextDocument, debounceTime?:number): Thenable<any> {
        if (document.languageId === ABL_MODE.language) {
            let ablDoc: LegacyABLDocument = this._documents[document.uri.fsPath];
    ...
    public getDocument(document: vscode.TextDocument): LegacyABLDocument {
        return this._documents[document.uri.fsPath];
    }

    private invokeUpdateDocument(ablDoc: LegacyABLDocument) {
        ablDoc.refreshDocument();
    }

    public broadcastDocumentChange(ablDoc: LegacyABLDocument) {
